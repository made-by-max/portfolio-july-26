...
![Illustration representing the balance between user experience and security in an authentication flow](https://miro.medium.com/v2/resize:fit:1100/format:webp/1*eHKR92uPSlwDRDac1rRQlw.png)
...
![MFA prompt](https://miro.medium.com/v2/resize:fit:720/format:webp/1*0Jzgk0GLaOWadP-DBh0ecg.png)
...
At Auth0 we work hard to ensure that our customers’ users don’t have to deal with CAPTCHA. Our automated attack protection analyzes traffic patterns in order to determine whether a login is being initiated by a human or a bot and only requires CAPTCHA if we’re reasonably certain traffic is coming from a bot. Like with Contextual MFA, a legitimate user’s login experience should never be interrupted.
...
![Login screen with an error prompt](https://miro.medium.com/v2/resize:fit:720/format:webp/1*KjWKa31diD72jDtzMYxvdA.png)
...
Using complex and unique passwords helps to prevent brute force attacks once a malicious actor has your username, but obfuscated error messages can prevent them from validating your username in the first place.
